...
const EXERCISES = [
  ...
  { id:"r7",set:"📘 Repaso Básico",title:"7. DrawSquare",
    ...
    zones:[{name:"Read-Host",check:c=>/Read-Host/i.test(c)},{name:"[int]",check:c=>/\[int\]/i.test(c)},{name:"Bucle (for/foreach/while)",check:c=>checkFlexLoop(c)},{name:"\"*\" * $n",check:c=>/"\*"\s*\*/.test(c)||/'\*'\s*\*/.test(c)},{name:"Salida en bucle",check:c=>/Write-(Host|Output)/i.test(c)}],
  ...
  { id:"m6",set:"🟡 Intermedio",title:"M6. Triángulo de *",description:"Pide N. Dibuja triángulo:\n*\n**\n***\n****\n*****",
    solution:`[int]$n = Read-Host "Numero"\nfor ($i = 1; $i -le $n; $i++) {\n    Write-Host ("*" * $i)\n}`,
    zones:[{name:"Read-Host",check:c=>/Read-Host/i.test(c)},{name:"Bucle (for/foreach/while)",check:c=>checkFlexLoop(c)},{name:"\"*\" * $i",check:c=>/"\*"\s*\*\s*\$/i.test(c)||/'\*'\s*\*\s*\$/i.test(c)},{name:"Salida",check:c=>/Write-(Host|Output)/i.test(c)}],
